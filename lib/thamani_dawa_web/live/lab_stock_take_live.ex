defmodule ThamaniDawaWeb.LabStockTakeLive do
  @moduledoc """
  Lab-portal stock take: start, resume, review, and finalize a physical count at an
  allowed (lab-capable) site. See `ThamaniDawa.StockTakes` for the domain logic and
  `ThamaniDawaWeb.StockTakeComponents` for the counting table shared with the pharmacy portal.
  """
  ...
  def mount(_params, _session, socket) do
    ...
    sites = organization_id |> Sites.list_sites() |> Enum.filter(&Site.lab?/1)
  ...
  def handle_event("start", %{"stock_take" => attrs}, socket) do
    ...
    case site_id &&
      ...
      {:ok, stock_take} ->
        {:noreply, push_navigate(socket, to: ~p"/lab/stock-take/#{stock_take.id}")}
    ...
    >
      <.header icon="hero-clipboard-document-check">
        ...
        <:actions>
          ...
          <.button :if={@live_action == :show} navigate={~p"/lab/stock-take"}>
            Back
          </.button>
        </:actions>
      </.header>
      ...
      <div :if={@live_action == :index}>
        <.table id="stock-takes" rows={@stock_takes}>
          ...
          <:action :let={st}>
            <.link navigate={~p"/lab/stock-take/#{st.id}"} class="link">

defmodule ThamaniDawaWeb.LabPatientLive.Index do
  ...
  defp handle_save_result({:ok, _visit}, socket) do
    {:noreply,
     ...
     |> reload_tab()
     |> push_patch(to: ~p"/lab/patients?tab=#{socket.assigns.tab}")}
  end
  ...
  def render(assigns) do
    ...
    <Layouts.lab_shell flash={@flash} current_scope={@current_scope} current_path="/lab/patients">
      <.header icon="hero-user-group">
        ...
        <:actions>
          <.button variant="primary" patch={~p"/lab/patients/new?tab=#{@tab}"}>
            + Log visit
          </.button>
        </:actions>
        <:toolbar>
      ...
      <.modal
        ...
        show
        on_cancel={JS.patch(~p"/lab/patients?tab=#{@tab}")}
      >
        ...
        >
          ...
          <div class="flex gap-2 mt-2">
            ...
            <.button patch={~p"/lab/patients?tab=#{@tab}"}>Cancel</.button>
          </div>
        </.form>
      </.modal>
      ...
      <.pagination page={@page_info} path={~p"/lab/patients?tab=#{@tab}"} />
    </Layouts.lab_shell>
    """
  end

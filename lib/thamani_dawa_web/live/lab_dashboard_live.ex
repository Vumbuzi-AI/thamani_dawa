defmodule ThamaniDawaWeb.LabDashboardLive do
  ...
  def mount(_params, _session, socket) do
    ...
    lab_orders = organization_id |> LabOrders.list_lab_orders() |> SiteScoping.for_current_site(scope)
  ...
  def render(assigns) do
    ...
    <Layouts.app_shell flash={@flash} current_scope={@current_scope}>
      ...
      <.table id="pending-orders" rows={@pending} row_click={&(~p"/lab/orders/#{&1.id}")}>
        <:col :let={lab_order} label="Patient">{patient_name(@patients_by_id, lab_order.patient_id)}</:col>
